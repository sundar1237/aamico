...
def addToSession(request):
    ...
    total_options = int(request.GET.get('total_options'))
    extra_price = float(request.GET.get('extra_price'))
    no_of_items = int(request.GET.get('no_of_items'))
    changed_total = float(request.GET.get('changed_total'))
    keys = request.GET
    extrasList = []
    for key in keys:
...
@api_view(['GET'])
# @permission_classes([IsAuthenticated])
def atc(request, exception=None):
    result = []
...
def checkout(request):
    ...
    for orderItem in orderItemkList:
        ...
        list.append({"s_no": count, "product": product, "no_of_items": no_of_items, "total_options": total_options,
                     "extra_price": extra_price, "changed_total": changed_total, "extrasString": extrasString,
...
def confirm_order(request):
    ...
    customer_nick_name = request.GET.get("customer_nick_name")
    deliveryType = request.GET['deliveryType']
    payMethod = request.GET['paymentMethod']
    orderItemList = request.session.get('orderItemList')
    print(
        f"customer nick name {customer_nick_name} delivery type {deliveryType} and pay method {payMethod} and total no of products {len(orderItemList)}")

    totalNoOfItems = 0
    price = 0

    order = Order(customer_nick_name=customer_nick_name, paymentStatus='Unpaid', deliveryType=deliveryType,
                  paymentMethod=payMethod, status="New", type="Direct")
    Order.save(order)
    count = 1
...
def view_order(request, pk):
    ...
    for orderItem in orderItemList:
        ...
        list.append(
            {"orderItem": orderItem, "extrasListString": prepareExtrasListString(extrasList, orderItem.extrasPrice),
             "product": product, })
...
def updateOrderStatus(request):
    ...
    if request.data:
        ...
        print(f"update order status > {json_data}")
